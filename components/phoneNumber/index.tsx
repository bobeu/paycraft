...
import { E164_REGEX } from "./twilioService";
...
//   let [componentInitialized, setComponentInitialized] = useState(false);
  ...
  const ISSUER_PRIVATE_KEY = String(process.env.NEXT_PUBLIC_ISSUER_PRIVATE_KEY);
  const DEK_PRIVATE_KEY = String(process.env.NEXT_PUBLIC_DEK_PRIVATE_KEY);
  ...
  async function registerIssuerAccountAndDEK() {
    ...
    // register DEK
    const DEK_PUBLIC_KEY = String(process.env.NEXT_PUBLIC_DEK_PUBLIC_KEY);
    ...
    try {
      ...
      if (accounts.length == 0) {
        const attestationReceipt = await federatedAttestationsContract
          .registerAttestationAsIssuer(identifier, formatAddr(address), verificationTime)
          .sendAndWaitForReceipt();
  ...
  async function deregisterPhoneNumber(phoneNumber: string) {
    try {
      ...
      const receipt = await federatedAttestationsContract
        .revokeAttestation(identifier, issuer.address, formatAddr(address))
        .sendAndWaitForReceipt();
      console.log(
